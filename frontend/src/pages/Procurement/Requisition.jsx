...
function Requisition() {
  ...
  const handleUpdateStatus = async (id, status) => {
    const res = await fetch(`http://localhost:5000/api/requisitions/updateRequisition/${id}/status`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async (id) => {
    await fetch(`http://localhost:5000/api/requisitions/deleteRequisition/${id}`, { method: "DELETE" });
